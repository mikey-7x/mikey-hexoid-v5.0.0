...
#             Industrial Self-Healing | STM32 LL/HAL Support | Multi-Format
#
# =============================================================================
...
install_deps_smart() {
    ...
    case $PKG_MGR in
        xbps)
            ...
            sudo xbps-install -S
            sudo xbps-install -y $PKGS 2>/dev/null
            ;;
        apt)
            PKGS="python3 python3-pip git curl wget unzip tar build-essential clang sdcc graphviz fontconfig libx11-dev libxft-dev iverilog"
            sudo apt update
            sudo apt install -y $PKGS
            ;;
        pacman)
            PKGS="python python-pip git curl wget unzip tar base-devel clang sdcc graphviz fontconfig libx11 libxft iverilog"
            sudo pacman -Syu --noconfirm
            sudo pacman -S --noconfirm --needed $PKGS
            ;;
        pkg)
            PKGS="python python-pip git curl wget unzip tar clang sdcc graphviz libx11 libxft iverilog"
            pkg update -y
            pkg install -y $PKGS
            ;;
        *)
...
BOARDS = {
    ...
    "49": ("Discovery F407VG", "STMicroelectronics:stm32:Disco:pnum=DISCO_F407VG"),
    "50": ("Discovery F303VC", "STMicroelectronics:stm32:Disco:pnum=DISCO_F303VC"),
...
def compile_arduino(path, fqbn, name):
    ...
    if "LED_BUILTIN" not in txt and "HAL_GPIO" not in txt and "LL_GPIO" not in txt:
...
# =============================================================================
# NEW: STM32 ADVANCED WORKFLOW (LL / HAL / Arduino)
# =============================================================================
def stm32_workflow():
    print(f"\n{UI.B}--- STM32 ADVANCED COMPILER (LL / HAL / Arduino) ---{UI.OFF}")
    print("1. LL (Low-Layer) Framework")
    print("2. HAL (Hardware Abstraction Layer) Framework")
    print("3. Arduino Standard Framework")
    print("0. Back")
    fw = input(f"{UI.C}Select Language Style > {UI.OFF}").strip()
    if fw not in ["1", "2", "3"]: return

    print("\n1. Create New STM32 Project")
    print("2. Compile Existing STM32 Project")
    act = input(f"{UI.C}Action > {UI.OFF}").strip()
    if act not in ["1", "2"]: return

    # Filter and display ONLY STM32 boards based on their backend architecture
    print(f"\n{UI.B}=== AVAILABLE STM32 TARGET BOARDS ==={UI.OFF}")
    stm_boards = {k: v for k, v in BOARDS.items() if "STMicroelectronics" in v[1]}
    for k, v in stm_boards.items():
        print(f" {k.rjust(3)}. {v[0]}")
    
    bid = input(f"\n{UI.C}Target Board ID > {UI.OFF}").strip()
    if bid not in stm_boards: return

    if act == "1":
        n = input(f"{UI.C}Project Name: {UI.OFF}").replace(" ", "_")
        if not n: return
        p = PROJECTS / n; p.mkdir(parents=True, exist_ok=True)
        f = p / f"{n}.ino"
        
        if fw == "1": # LL Boilerplate
        ...
        elif fw == "2": # HAL Boilerplate
            f.write_text("#include <Arduino.h>\n\nvoid setup() {\n  #if defined(STM32F1) || defined(STM32F4)\n    __HAL_RCC_GPIOC_CLK_ENABLE();\n  #endif\n  GPIO_InitTypeDef GPIO_InitStruct = {0};\n  GPIO_InitStruct.Pin = GPIO_PIN_13;\n  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;\n  GPIO_InitStruct.Pull = GPIO_NOPULL;\n  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;\n  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);\n}\n\nvoid loop() {\n  HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);\n  HAL_Delay(500);\n}")
        else: # Standard Arduino Boilerplate
            f.write_text("void setup() {\n  pinMode(LED_BUILTIN, OUTPUT);\n}\n\nvoid loop() {\n  digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));\n  delay(500);\n}")
        
        print(f"{UI.G}Project created for {stm_boards[bid][0]}. Modify pins if using Discovery/Nucleo.{UI.OFF}")
        subprocess.call([EDITOR, str(f)])

    elif act == "2":
        projs = get_project_list()
        if not projs:
            print(f"{UI.R}No projects found.{UI.OFF}")
            return
        for i,p in enumerate(projs): print(f" {i+1}. {p.name}")
        try:
            sel = int(input(f"{UI.C}Select Project Number > {UI.OFF}"))
            path = projs[sel-1]
            compile_arduino(path, stm_boards[bid][1], stm_boards[bid][0])
        except Exception as e:
            print(f"{UI.R}Error: {e}{UI.OFF}")
...
def help_ui():
    ...
    print(" 4. STM32 BluePill/BlackPill: PC13 (Active Low)")
...
{UI.B}3. STM32 ADVANCED WORKFLOW (Option 8){UI.OFF}
   - Dedicated flow to switch between Bare-Metal LL, HAL, or Arduino structures.
   - Provides targeted architecture pre-processor macros dynamically.

{UI.B}4. SPECIAL NOTES{UI.OFF}
...
def main():
    while True:
        ...
        print(" 8. STM32 Advanced (HAL / LL / Arduino)")
        print(" 0. Exit")
        c = input(f"\n{UI.C}mhex > {UI.OFF}").strip()
        if c=="1": create_ui()
        elif c=="2":
            projs = get_project_list()
            if not projs:
